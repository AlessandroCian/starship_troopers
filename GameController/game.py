...
while True:
	...
	# otherwise the tracker is tracking the face, update the position
		...
		if success:
			...
			cv2.circle(frame, (centerX, centerY), h//2, (0, 255, 0), 1)
	...
	if action is not None and end - start > wait_time:
		if not check_latest_action: #or latest_action != action:
			print(f"{action=} {latest_action=}")
	...
	cv2.putText(frame, action, (10, (0 * 20) + 20),
	cv2.QT_FONT_NORMAL, 0.6, (0, 0, 0), 2)
